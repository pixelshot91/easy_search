...
class _SearchScreenListState<T> extends State<SearchScreenList<T?>> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _waitingTimeToSearch = widget.filterPageSettings?.waitingTimeToSearch ?? 1500;
  ...
  //After page load, to try run the search
  void afeterPageLoad() {
    if (widget.filterPageSettings!.searchOnShow != null && widget.filterPageSettings!.searchOnShow == true)
      _tryToRunTheSearch();
  }
                      ...
                      : Padding(
                          ...
                          child: Container(
                            ...
                            child: MaterialButton(
                              ...
                              padding:
                                  widget.filterPageSettings!.filterField.sufixIconCircularPadding, //Clear X Padding
                              shape: CircleBorder(),
                            ),
                          ),
                        );
        ...
        return widget.customItemBuilder != null
            ...
            : Padding(
                ...
                child: InkWell(
                  ...
                  borderRadius:
                      BorderRadius.circular(widget.filterPageSettings!.buildItemFilter.circularBorderRadiusItem),
                  child: Padding(
                    padding: widget
                        .filterPageSettings!.buildItemFilter.spaceBetweenItems, //General Container Selected Padding
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(
            ...
            width: 0.0),
        borderRadius:
            BorderRadius.circular(widget.filterPageSettings!.buildItemFilter.circularBorderRadiusBackgroundItem),
      ),
      child: Padding(
    ...
    return AnimatedContainer(
      ...
      child: Row(
        ...
        children: [
          AnimatedContainer(
            duration:
                Duration(milliseconds: widget.filterPageSettings!.buildItemFilter.animationDurationMilliseconds + 200),
            curve: Curves.fastOutSlowIn,
            alignment: item.selected ? Alignment.center : AlignmentDirectional.bottomCenter,
            child: AnimatedContainer(
              duration: Duration(
                  milliseconds: widget.filterPageSettings!.buildItemFilter.animationDurationMilliseconds + 200),
  ...
  ScaleTransition buildSelectAll() {
    return ScaleTransition(
      ...
      alignment: FractionalOffset.center,
      child: Material(
        color: widget.filterPageSettings!.selectedAll.background
            .withOpacity(widget.filterPageSettings!.selectedAll.backgroundOpacity),
              ...
              decoration: BoxDecoration(
                ...
                border: widget.filterPageSettings!.selectedButton.border != null
                    ? widget.filterPageSettings!.selectedButton.border!
                        .withOpacity(widget.filterPageSettings!.selectedButton.borderOpacity) as BoxBorder?
                    : Border.all(
  ...
  configureSearchTimer() {
    ...
    if (_waitingTimeToSearch > 0) {
      ...
      _searchTimer = Timer(Duration(milliseconds: _waitingTimeToSearch), _doSearch);
    } else {
      _doSearch();
    }
  }
  ...
  void _doSearch() {
    if (_enableSearch &&
        widget.controller!.filter != null &&
        (widget.controller!.filter!.length > 0 || (widget.filterPageSettings?.searchOnEmpty ?? false))) {
      try {
  ...
  void executeOfflineSearch() {
    if (_enableSearch && widget.controller!.filter != null && widget.controller!.filter!.length > 0) {
      widget.controller!.getListItems!
          .where((element) =>
              element!.item == widget.controller!.filter ? element.visible = true : element.visible = false)
          .toList();
    } else {
